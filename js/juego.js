...
const Juego = {
    ...
    actualizarIA() {
    const rectJugador = UI.elementos.jugador.getBoundingClientRect();
    const rectArea = UI.elementos.gameArea.getBoundingClientRect();

    const centroJX = rectJugador.left - rectArea.left + rectJugador.width / 2;
    const centroJY = rectJugador.top - rectArea.top + rectJugador.height / 2;
    ...
    EstadoJuego.enemigos.forEach(enemigo => {
        ...
        if (distancia <= CONFIG.rangoAtaque) {
            ...
            EstadoJuego.jugador.vida = Math.max(
                0,
